...
const GITHUB_LIVE_COMPONENT_MAP = {
...
let DATES =
  Array.isArray(SEED_DATA.dates) && SEED_DATA.dates.length === TOTAL_DAYS
    ? SEED_DATA.dates
    : buildDates();
...
function liveOpenAIGroupStatus(components, groupName) {
  const relevant =
    groupName === "FedRAMP"
      ? (components || []).filter(function (component) {
          return /fedramp/i.test(component.name || "");
        })
      : (components || []).filter(function (component) {
          return (OPENAI_LIVE_COMPONENT_GROUPS[groupName] || []).includes(
            component.name,
          );
        });
  let worst = "g";
  relevant.forEach(function (component) {
    ...
    if ((STATUS_PRIORITY[code] || 0) > (STATUS_PRIORITY[worst] || 0))
      worst = code;
  });
  return worst;
}
...
function getGitHubDaily(name) {
  return getDisplayDaily(name, GITHUB_DAILY[name] || "");
}
...
function calcAvg(days) {
  ...
  function serviceAverage(daily) {
    ...
    for (let i = 0; i < slice.length; i++)
      total += UPTIME_SCORE[slice[i]] || 100;
    return total / slice.length;
  }
  const cAvg =
    RACES.reduce(
      (sum, race) => sum + serviceAverage(getClaudeDaily(race.c)),
      0,
    ) / RACES.length;
  const oAvg =
    RACES.reduce(
      (sum, race) => sum + serviceAverage(getOpenAIDaily(race.o)),
      0,
    ) / RACES.length;
  els.cAvg.textContent = cAvg.toFixed(2) + "%";
  els.oAvg.textContent = oAvg.toFixed(2) + "%";
  els.delta.textContent =
    (oAvg > cAvg ? "OpenAI" : "Claude") +
    " leads by " +
    Math.abs(cAvg - oAvg).toFixed(2) +
    "%";
}
...
function renderBars(data, side, serviceName) {
  ...
  const sourceData =
    ((side === "c"
      ? CLAUDE_DAILY[serviceName]
      : side === "g"
        ? GITHUB_DAILY[serviceName]
        : OPENAI_DAILY[serviceName]) || "") + "g".repeat(TOTAL_DAYS);
  return padded
    .slice(0, TOTAL_DAYS)
    .split("")
    .map(function (status, index) {
      ...
      const liveOverride =
        index === TOTAL_DAYS - 1 && status !== (sourceData[index] || "g");
      if (side === "c") {
        const detail =
          CLAUDE_DETAILS[serviceName] && CLAUDE_DETAILS[serviceName][date];
        const minutes =
          ((detail && detail.partialMinutes) || 0) +
          ((detail && detail.majorMinutes) || 0);
        if (detail && detail.events && detail.events.length)
          incidents = detail.events.join("||");
        if (minutes > 0) duration = fmtDuration(minutes);
      } else if (side === "g") {
        const detail =
          GITHUB_DETAILS[serviceName] && GITHUB_DETAILS[serviceName][date];
        const minutes =
          ((detail && detail.partialMinutes) || 0) +
          ((detail && detail.majorMinutes) || 0);
        if (detail && detail.events && detail.events.length)
        ...
        const detail =
          OPENAI_DETAILS[serviceName] && OPENAI_DETAILS[serviceName][date];
        if (detail && detail.titles && detail.titles.length)
          incidents = detail.titles.join("||");
      }
      return (
        '<div class="day ' +
        status +
        ...
        date +
        '" data-status="' +
        (STATUS_LABELS[status] || "Operational") +
        '" data-color="' +
        status +
        '" data-incidents="' +
        escAttr(incidents) +
...
function showTip(el) {
  ...
  if (el.dataset.ov) {
    ...
    services.forEach(function (service) {
      html +=
        '<div class="tip-svc"><span class="ts-dot" style="background:' +
        STATUS_COLORS[service.status] +
        '"></span>' +
        service.name +
        ": " +
        (STATUS_LABELS[service.status] || "OK") +
        "</div>";
    });
  } else {
    html +=
      '<div class="tip-status"><span class="td" style="background:' +
      STATUS_COLORS[color] +
      '"></span>' +
      (el.dataset.status || "Operational") +
      (el.dataset.duration ? " • " + el.dataset.duration : "") +
      "</div>";
    if (el.dataset.incidents) {
      ...
      el.dataset.incidents.split("||").forEach(function (item) {
        html +=
          '<div class="tip-incident"><span style="width:4px;height:4px;border-radius:50%;background:' +
          STATUS_COLORS[color] +
      ...
      html +=
        '<div style="font-size:.45rem;color:var(--muted);margin-top:.1rem">Live current status from provider summary.</div>';
    } else if (color === "g") {
      html +=
        '<div style="font-size:.45rem;color:var(--muted);margin-top:.1rem">No incidents recorded.</div>';
    }
  }
...
function bindDayInteractions() {
  function getTimelineCell(target) {
    return target.closest(".tl-cell");
  }
  function getCatRow(target) {
    return target.closest(".cat-row");
  }
  ...
  function showTimelineTip(el) {
    ...
    if (tlDayData) {
      ...
      cSvcs.forEach(function (s, si) {
        ...
        if (s.status !== "g")
          issues.push({ name: s.name, status: s.status, side: "c" });
      });
      oSvcs.forEach(function (s, si) {
        var sc = STATUS_SCORE[s.status] || 0;
        oScore += O_WEIGHTS[si] * sc;
        if (s.status !== "g")
          issues.push({ name: s.name, status: s.status, side: "o" });
      });
      html +=
        '<div style="display:flex;gap:.5rem;font-size:.48rem;margin-top:.2rem;color:var(--muted-lt)">';
      html +=
        '<span><span style="color:var(--claude);font-weight:600">Claude</span> ' +
        cScore.toFixed(1) +
        "/8</span>";
      html +=
        '<span><span style="color:var(--openai);font-weight:600">OpenAI</span> ' +
        oScore.toFixed(1) +
        "/8</span>";
      html += "</div>";
      if (issues.length > 0) {
        ...
        issues.forEach(function (iss) {
          ...
          html +=
            '<div style="font-size:.45rem;color:var(--muted-lt);display:flex;align-items:center;gap:4px;padding:1px 0"><span style="width:4px;height:4px;border-radius:50%;background:' +
            STATUS_COLORS[iss.status] +
            ';flex-shrink:0"></span>' +
            iss.name +
            ": " +
            (STATUS_LABELS[iss.status] || "OK") +
            "</div>";
        });
        html += "</div>";
      } else {
        html +=
          '<div style="font-size:.42rem;color:var(--muted);margin-top:.15rem">All services operational</div>';
      }
    }
    html +=
      '<div style="font-size:.42rem;color:var(--muted);margin-top:.15rem">Running: ' +
      score +
      "</div>";
  ...
  function showCatTip(el) {
    ...
    html +=
      '<div style="font-size:.5rem"><span style="color:var(--claude);font-weight:700">Claude</span><div style="font-size:.7rem;font-weight:700;color:var(--claude-lt)">' +
      cw +
      ' <span style="font-size:.45rem;color:var(--muted);font-weight:400">(' +
      cpct +
      "%)</span></div></div>";
    html +=
      '<div style="font-size:.5rem"><span style="color:var(--openai);font-weight:700">OpenAI</span><div style="font-size:.7rem;font-weight:700;color:var(--openai-lt)">' +
      ow +
      ' <span style="font-size:.45rem;color:var(--muted);font-weight:400">(' +
      opct +
      "%)</span></div></div>";
    html +=
      '<div style="font-size:.5rem"><span style="color:var(--muted);font-weight:600">Tied</span><div style="font-size:.7rem;font-weight:700;color:var(--muted-lt)">' +
      tie +
      ' <span style="font-size:.45rem;color:var(--muted);font-weight:400">(' +
      tpct +
      "%)</span></div></div>";
  ...
  document.addEventListener("click", function (event) {
    ...
    if (dayCell) {
      ...
      window.open(
        dayCell.dataset.href || getStatusPage(dayCell.dataset.side),
        "_blank",
        "noopener",
      );
      return;
    ...
    if (tlCell && !SUPPORTS_HOVER) {
      showTimelineTip(tlCell);
      return;
    }
    var catRow = getCatRow(event.target);
    if (catRow && !SUPPORTS_HOVER) {
      showCatTip(catRow);
      return;
    }
  ...
  document.addEventListener("mouseout", function (event) {
    if (
      getDayCell(event.target) ||
      getTimelineCell(event.target) ||
      getCatRow(event.target)
    )
      hideTip();
  });
}
...
function applyDays(days) {
  ...
  els.dayButtons.forEach(function (button) {
    button.classList.toggle(
      "active",
      parseInt(button.dataset.days, 10) === days,
    );
  });
  dataRows.forEach(function (row) {
    row.style.gridTemplateColumns = "repeat(" + days + ",1fr)";
    for (let i = 0; i < row.children.length; i++)
      row.children[i].hidden = i < skip;
  });
  calcAvg(days);
...
function buildOverview(cAgg, cPerDay, oAgg, oPerDay) {
  ...
  els.overview.innerHTML =
    '<div class="ov-row"><div class="ov-label">' +
    A_SM +
    ' Claude<span class="ov-tag">tracked services</span></div><div class="ov-bars">' +
    overviewBars(cAgg, cPerDay, "c") +
    "</div></div>" +
    '<div class="ov-row"><div class="ov-bars">' +
    overviewBars(oAgg, oPerDay, "o") +
    '</div><div class="ov-label">' +
    O_SM +
    " OpenAI</div></div>" +
...
function buildCards() {
  ...
  RACES.forEach(function (race, index) {
    ...
    card.innerHTML =
      '<div class="card-status"><div class="left"><span class="dot" style="background:' +
      winnerColor +
      '"></span><span>' +
      race.title +
      ...
      "</div></div>" +
      '<div class="card-bars">' +
      '<div class="brand-tag c">' +
      A_SM +
      " " +
      race.c +
      '<span style="margin-left:auto;font-size:.55rem;color:var(--muted);font-weight:400">' +
      cUptime.toFixed(2) +
      "%</span></div>" +
      '<div class="bar-row"><div class="bars">' +
      renderBars(cData, "c", race.c) +
      "</div></div>" +
      '<div class="bar-row"><div class="bars">' +
      renderBars(oData, "o", race.o) +
      "</div></div>" +
      '<div class="brand-tag o">' +
      O_SM +
      " " +
      race.o +
      '<span style="margin-left:auto;font-size:.55rem;color:var(--muted);font-weight:400">' +
      ...
      "%</span></div>" +
      "</div>" +
      '<div class="card-foot"><a href="' +
      STATUS_URLS.c +
      '" target="_blank" rel="noopener"><span class="sdot ' +
      dotCls(race.c, cData) +
      '"></span>' +
      race.c +
      ": " +
      statusTxt(race.c, cData) +
      '</a><a href="' +
      STATUS_URLS.o +
      '" target="_blank" rel="noopener">' +
      race.o +
      ": " +
      statusTxt(race.o, oData) +
      '<span class="sdot ' +
      dotCls(race.o, oData) +
      '"></span></a></div>';
...
function buildDataBlocks(
  ...
  const displayedClaude = Object.fromEntries(
    C_NAMES.map(function (name) {
      return [name, getClaudeDaily(name)];
    }),
  );
  const displayedOpenAI = Object.fromEntries(
    O_NAMES.map(function (name) {
      return [name, getOpenAIDaily(name)];
    }),
  );
  ...
  const o7 = last7.filter(function (v) {
    return v === "o";
  }).length;
  const c7 = last7.filter(function (v) {
    return v === "c";
  }).length;
  const t7 = last7.filter(function (v) {
    return v === "t";
  }).length;
  ...
  for (var i = skip; i < TOTAL_DAYS; i++) {
    ...
    C_NAMES.forEach(function (n, idx) {
      ...
      if ((STATUS_PRIORITY[s] || 0) > (STATUS_PRIORITY[cWorst] || 0))
        cWorst = s;
    });
    ...
    O_NAMES.forEach(function (n, idx) {
      ...
      if ((STATUS_PRIORITY[s] || 0) > (STATUS_PRIORITY[oWorst] || 0))
        oWorst = s;
    });
  ...
  var cTotalMins = 0;
  Object.keys(CLAUDE_MINUTES).forEach(function (k) {
    if (rangeDates[k]) cTotalMins += CLAUDE_MINUTES[k];
  });
  ...
  Object.keys(OAI_INCIDENTS).forEach(function (k) {
    if (rangeDates[k]) {
      oTotalIncidents += OAI_INCIDENTS[k].length;
      oIncidentDayCount++;
    }
  });
  ...
  for (var j = skip; j < TOTAL_DAYS; j++) {
    var allClean = true;
    C_NAMES.forEach(function (n) {
      if ((displayedClaude[n][j] || "g") !== "g") allClean = false;
    });
    O_NAMES.forEach(function (n) {
      if ((displayedOpenAI[n][j] || "g") !== "g") allClean = false;
    });
  ...
  catWins.forEach(function (cat) {
    ...
    var catLeader =
      cat.cW > cat.oW ? "Claude" : cat.oW > cat.cW ? "OpenAI" : "Tied";
    catHtml +=
      '<div class="cat-row" data-cat-title="' +
      cat.title +
      '" data-cat-cw="' +
      cat.cW +
      '" data-cat-ow="' +
      ...
      '">';
    catHtml +=
      '<div style="display:flex;justify-content:space-between;font-size:.45rem;color:var(--muted-lt);margin-bottom:.15rem"><span>' +
      cat.title +
      '</span><span><span class="c-hl">' +
      cat.cW +
      '</span> – <span class="o-hl">' +
      cat.oW +
      "</span> – " +
      cat.tie +
      "</span></div>";
    catHtml +=
      '<div class="db-bar"><div class="seg" style="flex:' +
      cat.cW +
      ';background:var(--claude)"></div><div class="seg" style="flex:' +
      cat.tie +
      ';background:var(--muted)"></div><div class="seg" style="flex:' +
      cat.oW +
      ';background:var(--openai)"></div></div>';
    catHtml += "</div>";
  });
  ...
  blocks +=
    '<div class="data-block"><div class="db-label">' +
    days +
    "-Day Leader</div>";
  blocks +=
    '<div class="db-value"><span class="' +
    leaderClass +
    '">' +
    leader +
    "</span></div>";
  ...
  blocks +=
    '<div class="data-block"><div class="db-label">Win Rate / Streak</div>';
  blocks += '<div class="db-split">';
  blocks +=
    '<div class="db-split-item"><div class="db-split-val c-hl">' +
    cWinRate +
    '%</div><div class="db-split-label">' +
    cStreak +
    "d streak</div></div>";
  blocks +=
    '<div class="db-split-item"><div class="db-split-val o-hl">' +
    oWinRate +
    '%</div><div class="db-split-label">' +
    oStreak +
    "d streak</div></div>";
  ...
  blocks +=
    '<div class="db-value">' +
    (last7Class
      ? '<span class="' + last7Class + '">' + last7Leader + "</span>"
      : last7Leader) +
    "</div>";
  blocks +=
    '<div class="db-detail"><span class="c-hl">' +
    c7 +
    '</span> – <span class="o-hl">' +
    o7 +
    "</span> – " +
  ...
  blocks +=
    '<div class="data-block"><div class="db-label">Biggest Comeback</div>';
  if (cbSide) {
    blocks +=
      '<div class="db-value"><span class="' +
      cbClass +
      '">' +
      bestComeback.droughtLen +
      "d</span></div>";
    blocks +=
      '<div class="db-detail"><span class="' +
      cbClass +
      '">' +
      cbSide +
      "</span> broke drought</div>";
  } else {
    blocks +=
      '<div class="db-value">—</div><div class="db-detail">No major droughts</div>';
  }
  blocks += "</div>";

  var cDaysAffected = 0;
  Object.keys(CLAUDE_MINUTES).forEach(function (k) {
    if (rangeDates[k]) cDaysAffected++;
  });
  blocks +=
    '<div class="data-block"><div class="db-label">Claude Downtime</div>';
  blocks += '<div class="db-value c-hl">' + cTotalHrs + "h</div>";
  blocks +=
    '<div class="db-detail">' + cDaysAffected + " days affected</div></div>";

  // === ROW 2: Category Breakdown (span 3) | Incidents/Clean | OpenAI Incidents ===
  blocks +=
    '<div class="data-block span-3"><div class="db-label">Category Breakdown</div>';
  blocks += '<div style="margin-top:.1rem">' + catHtml + "</div></div>";

  blocks +=
    '<div class="data-block"><div class="db-label">Incidents / Clean</div>';
  blocks += '<div class="db-split">';
  blocks +=
    '<div class="db-split-item"><div class="db-split-val c-hl">' +
    cIncidentDays +
    '</div><div class="db-split-label">Claude</div></div>';
  blocks +=
    '<div class="db-split-item"><div class="db-split-val o-hl">' +
    oIncidentDays +
    '</div><div class="db-split-label">OpenAI</div></div>';
  blocks +=
    '<div class="db-split-item"><div class="db-split-val">' +
    cleanDays +
    '</div><div class="db-split-label">Clean</div></div>';
  blocks += "</div>";
  blocks +=
    '<div class="db-detail" style="margin-top:.15rem"><span class="' +
    gapClass +
    '">' +
    gapWinner +
    "</span> leads " +
  ...
  blocks +=
    '<div class="data-block"><div class="db-label">OpenAI Incidents</div>';
  blocks += '<div class="db-value o-hl">' + oTotalIncidents + "</div>";
  blocks +=
    '<div class="db-detail">' +
    oIncidentDayCount +
    " days affected</div></div>";
...
function buildTimeline(dayWinners, runDiff, cPerDay, oPerDay) {
  ...
  for (var i = 0; i < dayWinners.length; i++) {
    ...
    var rd = runDiff[i];
    var scoreLabel =
      rd > 0 ? "Claude +" + rd : rd < 0 ? "OpenAI +" + Math.abs(rd) : "Even";
    cells +=
      '<div class="tl-cell ' +
      cls +
      '" data-tl-idx="' +
  ...
  return (
    '<div class="timeline-strip" style="grid-template-columns:repeat(' +
    dayWinners.length +
    ',1fr)">' +
    cells +
    "</div>"
...
function renderStandings(
  ...
  const dataBlocks = buildDataBlocks(
    dayWinners,
    cDaysWon,
    oDaysWon,
    tiedDays,
  ...
  els.standingsGrid.innerHTML =
    ...
    timeline +
    '<div class="timeline-meta"><span class="tl-time">' +
    numDays +
    ' days ago</span><span class="tl-time">Today</span></div>' +
    "</div>";
}
...
function computeAndRenderStandings(days) {
  ...
  const allClaude = C_NAMES.map(function (name) {
    return getClaudeDaily(name);
  });
  const allOpenAI = O_NAMES.map(function (name) {
    return getOpenAIDaily(name);
  });
  const cPerDay = [];
  const oPerDay = [];
  const dayWinners = [];
  ...
  for (let i = skip; i < TOTAL_DAYS; i++) {
    ...
    C_NAMES.forEach(function (name, index) {
      ...
      if ((STATUS_PRIORITY[status] || 0) > (STATUS_PRIORITY[cMax] || 0))
        cMax = status;
    ...
    O_NAMES.forEach(function (name, index) {
      ...
      if ((STATUS_PRIORITY[status] || 0) > (STATUS_PRIORITY[oMax] || 0))
        oMax = status;
  ...
  catWins.forEach(function (category) {
    if (
      Math.abs(category.oW - category.cW) >
      Math.abs(bigGapCat.oW - bigGapCat.cW)
    )
      bigGapCat = category;
    const oLeadsOverall = oDaysWon > cDaysWon;
    const trailerWins = oLeadsOverall ? category.cW : category.oW;
    const bestTrailerWins = oLeadsOverall
      ? trailerBestCat.cW
      : trailerBestCat.oW;
...
function buildGitHubSection() {
  ...
  const names = G_NAMES;
  const weights = G_WEIGHTS;
  const totalWeight = weights.reduce(function (a, b) {
    return a + b;
  }, 0);

  // Compute combined uptime
  let weightedSum = 0;
  names.forEach(function (name, idx) {
    const daily = getGitHubDaily(name);
    const score = UPTIME[name] || 100;
    weightedSum += weights[idx] * score;
  });
  const combinedUptime = (weightedSum / totalWeight).toFixed(2);

  // Compute current statuses from CURRENT_STATUS
  const statuses = names.map(function (name) {
    return CURRENT_STATUS[name] || "g";
  });
  const worstStatus = statuses.reduce(function (worst, s) {
    return (STATUS_PRIORITY[s] || 0) > (STATUS_PRIORITY[worst] || 0)
      ? s
      : worst;
  }, "g");
  ...
  for (var i = 0; i < TOTAL_DAYS; i++) {
    ...
    names.forEach(function (name) {
      var daily = getGitHubDaily(name);
      if ((daily[i] || "g") !== "g") hasIncident = true;
    });
    if (hasIncident) incidentDays++;
  }
  ...
  names.forEach(function (name, idx) {
    var daily = getGitHubDaily(name);
    var uptime = UPTIME[name] || 100;
    var bars = renderBars(daily, "g", name);
    var status = CURRENT_STATUS[name] || "g";
    var statusLabel = STATUS_LABELS[status] || "Operational";
    var dotColor = STATUS_COLORS[status] || "var(--green)";
    servicesHtml +=
      '<div class="gh-service">' +
      '<div class="gh-svc-head"><span class="gh-svc-dot" style="background:' +
      dotColor +
      '"></span>' +
      '<span class="gh-svc-name">' +
      name +
      "</span>" +
      '<span class="gh-svc-status">' +
      statusLabel +
      "</span>" +
      '<span class="gh-svc-uptime">' +
      uptime.toFixed(2) +
      "%</span></div>" +
      '<div class="bar-row"><div class="bars">' +
      bars +
  ...
  var dotColor = STATUS_COLORS[worstStatus] || "var(--green)";
  var statusLabel = STATUS_LABELS[worstStatus] || "Operational";

  body.innerHTML =
    '<div class="gh-summary">' +
    '<div class="gh-stat"><div class="gh-stat-val" style="color:var(--github-lt)">' +
    combinedUptime +
    '%</div><div class="gh-stat-label">Combined Uptime</div></div>' +
    '<div class="gh-stat"><div class="gh-stat-val"><span class="gh-svc-dot" style="background:' +
    dotColor +
    ';display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:4px"></span>' +
    statusLabel +
    '</div><div class="gh-stat-label">Current Status</div></div>' +
    '<div class="gh-stat"><div class="gh-stat-val">' +
    incidentDays +
    '</div><div class="gh-stat-label">Incident Days</div></div>' +
    "</div>" +
    servicesHtml;
}
...
function applyLiveData(data) {
  ...
  GITHUB_DAILY = { ...SEED_DATA.githubDaily, ...(data.githubDaily || {}) };
  UPTIME = { ...SEED_DATA.uptime, ...(data.uptime || {}) };
  OAI_INCIDENTS = data.oaiIncidents || {};
  GITHUB_INCIDENTS = data.githubIncidents || {};
  CLAUDE_MINUTES = data.claudeMinutes || {};
  CURRENT_STATUS = {
    ...SEED_DATA.currentStatus,
    ...(data.currentStatus || {}),
  };
  ...
  DATES =
    Array.isArray(data.dates) && data.dates.length === TOTAL_DAYS
      ? data.dates
      : buildDates();
}
...
async function refreshCurrentStatus(shouldRender) {
  try {
    const [claudeSummary, openaiSummary, githubSummary] = await Promise.all([
    ...
    (claudeSummary.components || []).forEach(function (component) {
      ...
      if (serviceName)
        nextStatus[serviceName] = API_STATUS_MAP[component.status] || "g";
    });
    Object.keys(OPENAI_LIVE_COMPONENT_GROUPS).forEach(function (groupName) {
      nextStatus[groupName] = liveOpenAIGroupStatus(
        openaiSummary.components || [],
        groupName,
      );
    });
    (githubSummary.components || []).forEach(function (component) {
      const serviceName = GITHUB_LIVE_COMPONENT_MAP[component.name];
      if (serviceName)
        nextStatus[serviceName] = API_STATUS_MAP[component.status] || "g";
    });
